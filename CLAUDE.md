...
This repo has submodules (`ondewo-proto-compiler`, `ondewo-s2t-api`). If a submodule's working
...
```text
Release ONDEWO S2T Python Client ${ONDEWO_S2T_VERSION}
```

So the heading of a new entry must read exactly `## Release ONDEWO S2T Python Client <version>`. **This wording is
...
```bash
grep -c '^## Release ONDEWO S2T Python Client ' RELEASE.md     # must be >= 1 for your new version
```
...
This package publishes as **`ondewo-s2t-client`**, which is not always the repository name — the JS client
...
```bash
curl -s https://pypi.org/pypi/ondewo-s2t-client/json | python3 -c "import sys,json;print(json.load(sys.stdin)['info']['version'])"
```
