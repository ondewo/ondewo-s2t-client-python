...
ONDEWO_S2T_API_GIT_BRANCH=OND211-2418-add-keycloak-for-2-fa
...
# `gh release create`. Harmless only while no entry used inline bold; the 7.4.1 entry does. Same fix as
# ondewo-nlu-client-python's Makefile.
CURRENT_RELEASE_NOTES=`cat RELEASE.md \
	| perl -ne 'print if /Release ONDEWO S2T Python Client ${ONDEWO_S2T_VERSION}/../^\*{5}/'`

GH_REPO="https://github.com/ondewo/ondewo-s2t-client-python"
DEVOPS_ACCOUNT_GIT="ondewo-devops-accounts"
DEVOPS_ACCOUNT_DIR="./${DEVOPS_ACCOUNT_GIT}"
ONDEWO_S2T_API_DIR=ondewo-s2t-api
ONDEWO_PROTO_COMPILER_DIR=ondewo-proto-compiler
GOOGLE_APIS_DIR=${ONDEWO_S2T_API_DIR}/googleapis
...
install_precommit_hooks: ## Installs pre-commit hooks and sets them up for the ondewo-s2t-client-python repo
	uv run pre-commit install
	uv run pre-commit install --hook-type commit-msg

precommit_hooks_run_all_files: ## Runs all pre-commit hooks on all files and not just the changed ones
	# --extra dev is required: pre-commit itself lives in the dev extra, and a bare `uv run` syncs only the
	# default dependency group — which would strip .venv back down and leave nothing to run. The mypy hook
	# is `language: system` (deliberately, so it can see the types-* packages) but resolves its interpreter
	# through `uv run --frozen --extra dev mypy`, so it works from any environment, this one included.
...
check_build: ## Checks if all built proto-code is there
	...
	@for proto in `find ondewo-s2t-api/ondewo -iname "*.proto*"`; \
	do \
...
update_setup: ## Update Version in pyproject.toml
	@perl -i -pe 's/^version = "[0-9]+\.[0-9]+\.[0-9]+"/version = "${ONDEWO_S2T_VERSION}"/' pyproject.toml
...
generate_services: ## Generate service wrapper files from proto definitions
	uv run python ondewo/s2t/scripts/generate_services.py \
...
show_pypi: build_package ## Shows PYPI Package in Dockerimage
	tar xvfz dist/ondewo_s2t_client-${ONDEWO_S2T_VERSION}.tar.gz
	tree ondewo_s2t_client-${ONDEWO_S2T_VERSION}
	cat ondewo_s2t_client-${ONDEWO_S2T_VERSION}/ondewo_s2t_client.egg-info/requires.txt
...
spc: ## Checks if the Release Branch, Tag and Pypi version already exist
	...
	# @if test "$(pyproject_version)" != "version = \"${ONDEWO_S2T_VERSION}\""; then echo "-- Test 3: pyproject.toml not updated!!"; exit 1; else echo "-- Test 3: pyproject.toml is fine";fi
