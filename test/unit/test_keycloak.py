...
class TestSharedProviderRegistry:
    ...
    def test_factory_uses_resolved_username_from_username_field(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        A config that sets only the primary `username` (legacy `user_name` left empty) must still send
        that username in the `grant_type=password` login form; regression guard for reading
        `config.user_name` directly instead of `config.resolved_username`.
    ...
    def test_configs_with_equal_credentials_share_one_provider(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        def build_config() -> ClientConfig:
            ...
            Returns:
                ClientConfig:
                    A fresh config object with the module-level test credentials.
            """
            return ClientConfig(
                host="localhost",
                port="50055",
    ...
    def test_configs_with_different_credentials_never_share_a_provider(
        ...
        def build_config(user_name: str) -> ClientConfig:
            ...
            Args:
                user_name (str):
                    The identity the config authenticates as.

            Returns:
                ClientConfig:
                    A fresh config object naming that identity.
            """
            return ClientConfig(
                host="localhost",
                port="50055",
...
from ondewo.s2t.client.utils.keycloak import KeycloakTokenProvider
