...
def _emit_method(
    ...
    req_name = 'request_iterator' if rpc.client_streaming else 'request'
    sig_single = f'    def {method}(self, {req_name}: {req}) -> {resp}:'
    body_single = f'        response: {resp} = self.stub.{rpc.name}({req_name})'  # , metadata=self.metadata)'
    ...
    if len(body_single) <= _MAX_LINE_LENGTH:
        ...
        out += [
            f'        response: {resp} = \\',
            f'            self.stub.{rpc.name}({req_name}, metadata=self.metadata)',
        ]
...
def _build_file_content(svc: ServiceDef, type_to_stem: Dict[str, str]) -> str:
