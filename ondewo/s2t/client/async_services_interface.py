...
"""Async services base that injects the Keycloak bearer token into every S2T gRPC call.

The generated async service wrappers (``ondewo/s2t/client/services/async_*.py``) subclass
:class:`AsyncServicesInterface` and forward :attr:`AsyncServicesInterface.metadata` to every
stub call. When the :class:`~ondewo.s2t.client.client_config.ClientConfig` opts into the D18
